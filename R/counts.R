...
.as_sequence_character <- function(sequences) {
    if (is.character(sequences)) {
        ...
        stop(
            "`sequences` must be a character vector or a Biostrings XStringSet.",
            call. = FALSE
        )
    }
...
selexprep_count <- function(sequences) {
    ...
    if (!length(values)) {
        return(S4Vectors::DataFrame(
            sequence = Biostrings::BStringSet(),
            reads = numeric(),
            rank = integer(),
            rpm = numeric()
        ))
    ...
    S4Vectors::DataFrame(
        sequence = Biostrings::BStringSet(unique_values),
        reads = reads,
        rank = seq_along(reads),
        rpm = reads / n_reads * 1e6
    )
}
...
.as_selexprep_experiment <- function(counts_by_round, accession = NULL) {
    if (!is.list(counts_by_round) || !length(counts_by_round)) {
        stop("`counts_by_round` must be a non-empty named list of count tables.", call. = FALSE)
    }
    ...
    sequences_by_round <- lapply(counts_by_round, function(counts) {
        if (!inherits(counts, "DataFrame") ||
            !all(c("sequence", "reads") %in% colnames(counts))) {
            stop(
                "Each element of `counts_by_round` must be a DataFrame from `selexprep_count()`.",
                call. = FALSE
            )
        }
    ...
    row_index <- unlist(lapply(sequences_by_round, match, table = all_sequences), use.names = FALSE)
    ...
    counts <- Matrix::sparseMatrix(
        ...
        j = col_index,
        x = count_values,
        dims = c(length(all_sequences), length(counts_by_round)),
        dimnames = list(all_sequences, round_names)
    )

    SummarizedExperiment::SummarizedExperiment(
        assays = list(counts = counts),
        rowData = S4Vectors::DataFrame(
            sequence = Biostrings::BStringSet(all_sequences),
            row.names = all_sequences
        ),
        colData = S4Vectors::DataFrame(round = round_names, row.names = round_names),
        metadata = list(accession = accession, schema_version = "0.1.0")
    )
}
