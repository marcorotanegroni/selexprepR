.round_patterns <- list(
    round_word_digit = "(?i)\\bround[[:space:]_\\-]*(\\d+)\\b",
    cycle_word_digit = "(?i)\\bcycle[[:space:]_\\-]*(\\d+)\\b",
    iteration_digit = "(?i)\\biteration[[:space:]_\\-]*(\\d+)\\b",
    pool_digit = "(?i)\\bpool[[:space:]_\\-]*(\\d+)\\b",
    r_digit_boundary = "(?i)(?<![[:alnum:]])r(\\d+)(?![[:alnum:]])",
    c_digit_boundary = "(?i)(?<![[:alnum:]])c(\\d+)(?![[:alnum:]])",
    digit_r_suffix = "(?i)(?<![[:alnum:]])(\\d+)r(?![[:alnum:]])",
    rv_digit = "(?i)(?<![[:alnum:]])rv(\\d+)(?![[:alnum:]])",
    glued_word_r_digit = "[A-Z][A-Za-z]{2,}R(\\d+)(?![[:alnum:]])",
...
.target_hint <- function(sample_title) {
    ...
    if (length(found) >= 2L) found[[2L]] else NA_character_
}
...
.single_round_assignment <- function(run_accession, sample_title = "", library_name = "",
    ...
    override <- if (run_accession %in% names(overrides)) overrides[[run_accession]] else NA_integer_
    if (!is.null(override) && !is.na(override)) {
        return(list(
            round_number = as.integer(override), confidence = "HIGH",
            source_field = "seed_override", matched_pattern = "manual",
            candidates = as.integer(override),
            parser_notes = sprintf("overridden by supplied mapping: round=%d", as.integer(override)),
    ...
    for (key in names(attributes)) {
        ...
        if (grepl(.round_attribute_key, trimws(key), perl = TRUE) && grepl("^\\d+$", value)) {
            round <- as.integer(value)
            return(list(
                round_number = round, confidence = "HIGH",
                source_field = "sample_attributes", matched_pattern = "structured_attr",
                candidates = round,
                parser_notes = sprintf("structured attribute key='%s' value='%s'", key, value),
                target_hint = .target_hint(sample_title)
            ))
        }
    }
    levels <- list(
        list(name = "sample_title", text = sample_title, confidence = "HIGH"),
    ...
    for (level in levels) {
        ...
        confidence <- if (length(candidates) == 1L) level$confidence else "MEDIUM"
        notes <- if (length(candidates) == 1L) {
            sprintf("matched '%s' in %s: '%s'", patterns[[1L]], level$name,
                substr(level$text, 1L, 120L))
        } else {
            sprintf("conflicting candidates %s from patterns %s in %s: '%s'",
                paste(candidates, collapse = ","), paste(patterns, collapse = ","), level$name,
                substr(level$text, 1L, 120L))
        }
        return(list(
            round_number = as.integer(candidates[[1L]]), confidence = confidence,
            source_field = level$name, matched_pattern = patterns[[1L]],
            candidates = as.integer(candidates), parser_notes = notes,
            target_hint = .target_hint(sample_title)
        ))
    }
    list(
        round_number = NA_integer_, confidence = "NONE", source_field = "none",
        matched_pattern = "none", candidates = integer(),
        parser_notes = "no round indicator found in supplied metadata fields",
        target_hint = .target_hint(sample_title)
    )
}
...
infer_selexprep_rounds <- function(runs, overrides = numeric()) {
    ...
    if (!is.numeric(overrides) || (length(overrides) &&
        (is.null(names(overrides)) || any(!nzchar(names(overrides))) || anyNA(overrides) ||
            any(overrides < 0) || any(overrides != floor(overrides))))) {
    ...
    assignments <- lapply(seq_len(nrow(runs)), function(index) {
        .single_round_assignment(
            accession[[index]],
            sample_title = .round_column(runs, "sample_title")[[index]],
            library_name = .round_column(runs, "library_name")[[index]],
    ...
    S4Vectors::DataFrame(
        ...
        round_number = vapply(assignments, `[[`, integer(1), "round_number"),
        confidence = vapply(assignments, `[[`, character(1), "confidence"),
        source_field = vapply(assignments, `[[`, character(1), "source_field"),
        matched_pattern = vapply(assignments, `[[`, character(1), "matched_pattern"),
        round_candidates = I(lapply(assignments, `[[`, "candidates")),
...
.safe_round_assignments <- function(assignments) {
    !is.na(assignments$round_number) & lengths(assignments$round_candidates) == 1L
}
