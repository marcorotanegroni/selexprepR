...
.parse_ena_filereport <- function(text, accession) {
    ...
    if (!nrow(raw)) {
        stop(sprintf("ENA returned no read-run records for accession %s.", accession), call. = FALSE)
    }
    missing <- setdiff(.ena_filereport_fields, colnames(raw))
    if (length(missing)) {
        stop(
            sprintf("ENA response is missing required fields: %s.", paste(missing, collapse = ", ")),
            call. = FALSE
        )
    }
    runs <- S4Vectors::DataFrame(
        run_accession = vapply(raw$run_accession, .as_ena_scalar, character(1)),
        sample_accession = vapply(raw$sample_accession, .as_ena_scalar, character(1)),
        sample_title = vapply(raw$sample_title, .as_ena_scalar, character(1)),
        library_name = vapply(raw$library_name, .as_ena_scalar, character(1)),
        experiment_title = vapply(raw$experiment_title, .as_ena_scalar, character(1)),
        read_count = suppressWarnings(as.numeric(raw$read_count)),
        base_count = suppressWarnings(as.numeric(raw$base_count)),
        fastq_urls = I(lapply(raw$fastq_ftp, .ena_semicolon_values)),
        fastq_md5 = I(lapply(raw$fastq_md5, .ena_semicolon_values)),
        fastq_bytes = I(lapply(raw$fastq_bytes, .ena_semicolon_values, numeric = TRUE))
    )
    first <- raw[1L, , drop = FALSE]
    structure(list(
        accession = accession,
        bioproject_id = .as_ena_scalar(first$study_accession),
        study_title = .as_ena_scalar(first$study_title),
        library_strategy = .as_ena_scalar(first$library_strategy),
        library_source = .as_ena_scalar(first$library_source),
        runs = runs
    ), class = c("selexprep_inspection", "list"))
}
...
.fetch_plan <- function(inspection, overrides = numeric()) {
    ...
    rows <- lapply(seq_len(nrow(inspection$runs)), function(index) {
        ...
        n <- length(urls)
        data.frame(
            run_accession = rep(inspection$runs$run_accession[[index]], n),
            round_number = rep(assignments$round_number[[index]], n),
            round_confidence = rep(assignments$confidence[[index]], n),
            round_source = rep(assignments$source_field[[index]], n),
            round_unambiguous = rep(length(assignments$round_candidates[[index]]) == 1L, n),
            file_name = basename(sub("\\?.*$", "", urls)),
            url = ifelse(
                grepl("^https?://", urls),
                urls,
                paste0("https://", sub("^ftp://", "", urls))
            ),
            expected_md5 = c(md5, rep(NA_character_, n))[seq_len(n)],
            expected_bytes = c(bytes, rep(NA_real_, n))[seq_len(n)],
            stringsAsFactors = FALSE
        )
    })
    ...
    if (anyDuplicated(plan$file_name)) {
        stop("ENA returned duplicated FASTQ basenames; refusing an ambiguous download plan.", call. = FALSE)
    }
    plan
}
...
inspect_selexprep_accession <- function(accession, timeout_seconds = 30) {
    if (!is.character(accession) || length(accession) != 1L || is.na(accession) || !nzchar(accession)) {
    ...
    request <- httr2::req_url_query(
        ...
        accession = accession,
        result = "read_run",
        fields = paste(.ena_filereport_fields, collapse = ","),
        format = "tsv"
    )
    response <- httr2::req_perform(httr2::req_timeout(request, timeout_seconds))
    if (httr2::resp_status(response) >= 400L) {
        stop(
            sprintf("ENA request failed with HTTP status %d.", httr2::resp_status(response)),
            call. = FALSE
        )
    }
...
#' @param timeout_seconds Positive timeout forwarded when an accession is given.
...
fetch_selexprep_reads <- function(accession_or_inspection, outdir, dry_run = TRUE,
    overwrite = FALSE, timeout_seconds = 30, require_assigned_rounds = TRUE,
    overrides = numeric()) {
    ...
    if (!is.logical(dry_run) || length(dry_run) != 1L || is.na(dry_run) ||
        ...
        !is.logical(require_assigned_rounds) || length(require_assigned_rounds) != 1L ||
        is.na(require_assigned_rounds)) {
    ...
    if (require_assigned_rounds) {
        ...
        if (any(unsafe)) {
            stop(
                sprintf(
                    "Refusing download: no unambiguous SELEX round assignment for %s. Supply `overrides` after manual review or set `require_assigned_rounds = FALSE` for acquisition only.",
                    paste(unique(plan$run_accession[unsafe]), collapse = ", ")
                ),
                call. = FALSE
            )
        }
    ...
    for (index in seq_len(nrow(plan))) {
        ...
        if (file.exists(destination) && !overwrite) {
            stop(sprintf("Refusing to overwrite existing file: %s", destination), call. = FALSE)
        }
        temporary <- tempfile(pattern = ".selexprep-download-", tmpdir = outdir)
        status <- utils::download.file(plan$url[[index]], temporary,
            method = "libcurl", mode = "wb", quiet = TRUE
        )
        if (!identical(status, 0L)) {
            stop(sprintf("Download failed for %s.", plan$url[[index]]), call. = FALSE)
        }
        ...
        if (!is.na(expected_md5)) {
            ...
            if (!identical(tolower(observed_md5), tolower(expected_md5))) {
                stop(sprintf("MD5 verification failed for %s; temporary file retained at %s.",
                    plan$file_name[[index]], temporary
                ), call. = FALSE)
            }
        }
        if (!file.rename(temporary, destination)) {
            stop(sprintf("Could not move verified download to %s.", destination), call. = FALSE)
        }
