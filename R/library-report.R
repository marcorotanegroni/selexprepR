.library_report_fields <- c(
    "primer_5p", "primer_3p", "variants_5p", "variants_3p",
    "known_adapter_hits", "extraction_mode", "full_insert_recovered",
    "read_source", "required_action", "orientation", "n_length_mode",
    "n_length_distribution", "n_length_confidence", "match_rate_5p",
    "match_rate_3p", "position_consistency_5p", "position_consistency_3p",
    "read_fraction_used_for_inference", "sampling_seed", "confidence",
    "status", "failure_reason"
)

.extraction_modes <- c(
    "BOTH_PRIMERS_SINGLE_READ", "FIVE_PRIME_ONLY", "THREE_PRIME_ONLY",
...
.new_library_report <- function(fields) {
    ...
    for (name in c("extraction_mode", "read_source", "required_action", "orientation", "status")) {
        allowed <- switch(name,
        ...
        if (!is.character(fields[[name]]) || length(fields[[name]]) != 1L ||
            !(fields[[name]] %in% allowed)) {
    ...
    if (!is.logical(fields$full_insert_recovered) || length(fields$full_insert_recovered) != 1L ||
        ...
        stop("`full_insert_recovered` must be one non-missing logical value.", call. = FALSE)
    }
    numeric_fields <- c(
        "n_length_confidence", "match_rate_5p", "match_rate_3p",
        "position_consistency_5p", "position_consistency_3p",
        "read_fraction_used_for_inference", "sampling_seed", "confidence"
    )
    if (any(!vapply(fields[numeric_fields], function(x) is.numeric(x) && length(x) == 1L &&
        !is.na(x), logical(1)))) {
        stop("LibraryReport numeric scalar fields must be non-missing scalars.", call. = FALSE)
    }
    if (!is.null(fields$n_length_mode) &&
        (!is.numeric(fields$n_length_mode) || length(fields$n_length_mode) != 1L)) {
        stop("`n_length_mode` must be NULL or a numeric scalar.", call. = FALSE)
    }
    if (!is.numeric(fields$n_length_distribution) ||
        (length(fields$n_length_distribution) && is.null(names(fields$n_length_distribution)))) {
        stop("`n_length_distribution` must be a named numeric vector.", call. = FALSE)
    }
    if (!is.numeric(fields$known_adapter_hits) ||
        (length(fields$known_adapter_hits) && is.null(names(fields$known_adapter_hits)))) {
...
.unable_library_report <- function(read_source, sampling_seed, failure_reason,
    ...
    .new_library_report(list(
        primer_5p = NULL,
        primer_3p = NULL,
        variants_5p = list(),
        variants_3p = list(),
        known_adapter_hits = known_adapter_hits,
        ...
        orientation = "FORWARD",
        n_length_mode = NULL,
        n_length_distribution = numeric(),
        n_length_confidence = 0,
        match_rate_5p = 0,
        match_rate_3p = 0,
        position_consistency_5p = 0,
        position_consistency_3p = 0,
        read_fraction_used_for_inference = 0,
        sampling_seed = as.numeric(sampling_seed),
        confidence = 0,
...
.hamming_leq_one <- function(left, right) {
    ...
    sum(strsplit(left, "", fixed = TRUE)[[1L]] != strsplit(right, "", fixed = TRUE)[[1L]]) <= 1L
}
...
.flank_boundary <- function(supports, min_length, confidence) {
    ...
    for (i in seq.int(min_length + 1L, max(min_length + 1L, length(supports) - 1L))) {
        if (i + 1L > length(supports)) {
            break
        }
        baseline <- stats::median(supports[(i - min_length):(i - 1L)])
        window <- supports[i:(i + 1L)]
        if (baseline >= 0.90 && supports[[i]] <= baseline - 0.12 && all(window < 0.85)) {
            boundary <- i - 1L
            break
        }
    }
    ...
    if (mean(called >= confidence) < 0.80) {
        return(0L)
    }
    as.integer(boundary)
}
...
.detect_flank <- function(sequences, is_prefix, max_length = 60L, min_length = 14L,
    ...
    primer <- paste0(if (is_prefix) bases else rev(bases), collapse = "")
    widths <- nchar(sequences)
    fragments <- if (is_prefix) {
        substr(sequences[widths >= primer_length], 1L, primer_length)
    } else {
        substr(sequences[widths >= primer_length],
            widths[widths >= primer_length] - primer_length + 1L,
            widths[widths >= primer_length])
    }
    ...
    list(
        sequence = primer,
        length = as.integer(primer_length),
        confidence = mean(vapply(fragments, .hamming_leq_one, logical(1), right = primer))
    )
}

.detect_primers <- function(sequences, confidence = 0.75, max_length = 60L,
    min_length = 14L, min_sequences = 500L) {
    if (length(sequences) < min_sequences) {
        return(NULL)
    }
    primer_5p <- .detect_flank(sequences, TRUE, max_length, min_length, confidence, min_sequences)
    primer_3p <- .detect_flank(sequences, FALSE, max_length, min_length, confidence, min_sequences)
    list(
        primer_5p = primer_5p,
        primer_3p = primer_3p,
        n_sequences_analyzed = length(sequences),
        mean_sequence_length = as.integer(round(mean(nchar(sequences)))),
        estimated_random_region_length = if (is.null(primer_5p$sequence) &&
...
.adapter_reverse_complements <- function() {
    stats::setNames(vapply(.known_adapters, reverse_complement, character(1)), names(.known_adapters))
}
...
.matches_known_adapter_prefix <- function(primer) {
    ...
    any(vapply(.known_adapters, function(adapter) substr(adapter, 1L, .adapter_probe_length) == probe,
        logical(1))) || any(vapply(reverse_adapters, function(adapter) {
...
.count_adapter_hits <- function(sequences) {
    ...
    hits <- stats::setNames(vapply(names(.known_adapters), function(name) {
        ...
        sum(grepl(forward, sequences, fixed = TRUE) | grepl(reverse, sequences, fixed = TRUE))
...
.top_variants <- function(sequences, length, is_prefix, k = 3L) {
    ...
    fragments <- if (is_prefix) {
        ...
        substr(sequences[widths >= length], widths[widths >= length] - length + 1L,
    ...
    hits <- vapply(usable, function(sequence) {
        ...
        for (offset in 0L:tolerance) {
            fragment <- if (is_prefix) {
                if (offset + primer_length > sequence_length) next
                substr(sequence, offset + 1L, offset + primer_length)
            } else {
                if (primer_length + offset > sequence_length) next
                substr(sequence, sequence_length - primer_length - offset + 1L, sequence_length - offset)
            }
...
.substring_match_rate <- function(sequences, primer) {
    ...
    mean(vapply(usable, function(sequence) {
        ...
        any(vapply(seq_len(sequence_length - primer_length + 1L), function(start) {
            .hamming_leq_one(substr(sequence, start, start + primer_length - 1L), primer)
        }, logical(1)))
    }, logical(1)))
}
...
.primers_agree <- function(left, right) {
    ...
    .contains_hamming_leq_one(if (nchar(left) > nchar(right)) left else right,
        if (nchar(left) > nchar(right)) right else left)
}
...
.n_length_stats <- function(sequences, primer_5p_length, primer_3p_length) {
    ...
    index <- which.max(counts)
    list(
        mode = as.numeric(values[[index]]),
        distribution = stats::setNames(as.numeric(counts), as.character(values)),
        confidence = counts[[index]] / sum(counts)
    )
}
...
.detect_orientation <- function(sequences, primer_5p, primer_3p) {
    ...
    for (sequence in sequences) {
        if (!is.null(primer_5p) && nchar(sequence) >= nchar(primer_5p) &&
            .hamming_leq_one(substr(sequence, 1L, nchar(primer_5p)), primer_5p)) {
            forward <- forward + 1L
    ...
    if (reverse / total > 0.95) "REVERSE" else "MIXED"
}

.composite_confidence <- function(signals, has_round_map) {
    weights <- if (has_round_map) {
        c(match_5p = 0.15, match_3p = 0.15, pos_5p = 0.15, pos_3p = 0.15,
            persistence = 0.25, n_length = 0.10, adapter_clean = 0.05)
    } else {
        c(match_5p = 0.225, match_3p = 0.225, pos_5p = 0.225, pos_3p = 0.225,
            persistence = 0, n_length = 0.05, adapter_clean = 0.05)
    }
...
.assign_status <- function(extraction_mode, confidence, has_round_map) {
    ...
    if (!has_round_map && identical(status, "HIGH")) "MEDIUM" else status
}
...
.classify_library <- function(match_rate_5p, match_rate_3p, n_length_confidence,
    ...
    if (match_rate_5p < 0.4 && match_rate_3p < 0.4) {
        return(list(
            extraction_mode = "UNABLE_TO_EXTRACT", full_insert_recovered = FALSE,
    ...
    if (has_paired_split) {
        ...
        return(list(
            extraction_mode = mode, full_insert_recovered = FALSE,
    ...
    if (match_rate_5p > 0.7 && match_rate_3p > 0.7) {
        ...
        return(list(
            extraction_mode = mode, full_insert_recovered = TRUE, required_action = "NONE",
            status = .assign_status(mode, confidence, has_round_map), failure_reason = NULL
        ))
    }
    if (match_rate_5p > 0.7 || match_rate_3p > 0.7) {
        if (n_length_confidence > 0.8) {
            mode <- if (match_rate_5p > 0.7) "FIVE_PRIME_ONLY" else "THREE_PRIME_ONLY"
            return(list(
                extraction_mode = mode, full_insert_recovered = FALSE, required_action = "NONE",
                status = .assign_status(mode, confidence, has_round_map), failure_reason = NULL
            ))
        }
        side <- if (match_rate_5p > 0.7) "5'" else "3'"
        return(list(
            extraction_mode = "UNABLE_TO_EXTRACT", full_insert_recovered = FALSE,
            required_action = "MANUAL_PRIMERS_REQUIRED", status = "UNABLE_TO_INFER",
...
.as_round_pools <- function(sequences_by_round, argument) {
    if (!is.list(sequences_by_round)) {
        stop(sprintf("`%s` must be a list of per-round sequence pools.", argument), call. = FALSE)
    }
    ...
    if (any(!nzchar(round_names)) || anyDuplicated(round_names)) {
        stop(sprintf("`%s` must have unique, non-empty round names.", argument), call. = FALSE)
    }
...
.subsample_pool <- function(sequences, max_reads) {
    ...
    list(
        sequences = sequences[sample.int(length(sequences), max_reads)],
        fraction = max_reads / length(sequences)
    )
}
...
selexprep_detect <- function(sequences_by_round,
    read_source = c("R1", "R2", "R1_AND_R2", "INTERLEAVED", "UNKNOWN"),
    paired_mate_streams = NULL, sampling_seed = 42L, max_reads_per_round = NULL) {
    ...
    if (!is.null(max_reads_per_round) &&
        (!is.numeric(max_reads_per_round) || length(max_reads_per_round) != 1L ||
            is.na(max_reads_per_round))) {
    ...
    mate_pools <- if (is.null(paired_mate_streams)) NULL else {
    ...
    .with_sampling_seed(sampling_seed, {
        fractions <- numeric(length(pools))
        normalised <- lapply(seq_along(pools), function(index) {
            sampled <- .subsample_pool(.normalise_pool(pools[[index]]), max_reads_per_round)
            fractions[[index]] <<- sampled$fraction
            sampled$sequences
        })
        ...
        if (is.null(detection)) {
            return(.unable_library_report(
                read_source, sampling_seed,
                "Earliest round has fewer than 500 sequences \u2014 below detection floor",
                adapter_hits
            ))
        }
        ...
        if (adapter_drop_5p) primer_5p <- NULL
        if (adapter_drop_3p) primer_3p <- NULL
        ...
        persistence <- .combine_persistence(
            .persistence_score(position_rates_5p), .persistence_score(position_rates_3p),
            primer_5p, primer_3p
        )
        ...
        p5_length <- if (is.null(primer_5p)) 0L else nchar(primer_5p)
        p3_length <- if (is.null(primer_3p)) 0L else nchar(primer_3p)
        p3_lookup_length <- if (is.null(primer_3p_lookup)) 0L else nchar(primer_3p_lookup)
        ...
        confidence <- .composite_confidence(list(
            match_5p = match_rate_5p,
            match_3p = match_rate_3p,
            pos_5p = position_rates_5p[[1L]],
            pos_3p = position_rates_3p[[1L]],
            persistence = persistence,
            n_length = n_lengths$confidence,
            adapter_clean = if (adapter_drop_5p || adapter_drop_3p) 0 else 1
        ), has_round_map)
        classification <- .classify_library(
            match_rate_5p, match_rate_3p, n_lengths$confidence,
            has_paired_split, has_round_map, confidence
        )
        .new_library_report(list(
            primer_5p = primer_5p,
            ...
            n_length_mode = n_lengths$mode,
            n_length_distribution = n_lengths$distribution,
            n_length_confidence = n_lengths$confidence,
            match_rate_5p = match_rate_5p,
            match_rate_3p = match_rate_3p,
            position_consistency_5p = position_rates_5p[[1L]],
            position_consistency_3p = position_rates_3p[[1L]],
            read_fraction_used_for_inference = mean(fractions),
            sampling_seed = as.numeric(sampling_seed),
...
.library_report_payload <- function(report) {
    ...
    payload$known_adapter_hits <- payload$known_adapter_hits[order(names(payload$known_adapter_hits), method = "radix")]
...
write_library_report <- function(report, path) {
    ...
    json <- sub('"known_adapter_hits": []', '"known_adapter_hits": {}', json, fixed = TRUE)
    json <- sub('"n_length_distribution": []', '"n_length_distribution": {}', json, fixed = TRUE)
    # Python's schema writes float-valued signals as `1.0` rather than `1`.
    # Keep that stable spelling and its 16-significant-digit representation so
    # JSON produced by both implementations is byte-identical for reference
    # reports.
    float_fields <- c(
        "n_length_confidence", "match_rate_5p", "match_rate_3p",
        "position_consistency_5p", "position_consistency_3p",
        "read_fraction_used_for_inference", "confidence"
    )
    ...
    for (field in float_fields) {
        pattern <- sprintf('(\\"%s\\":\\s*)-?[0-9]+(?:\\.[0-9]+)?(?:[eE][+-]?[0-9]+)?', field)
        json <- sub(pattern, paste0("\\1", format_python_float(payload[[field]])), json, perl = TRUE)
    }
    text <- paste0(json, "\n")
