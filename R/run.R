...
run_selexprep <- function(sequences_by_round,
    read_source = c("R1", "R2", "R1_AND_R2", "INTERLEAVED", "UNKNOWN"),
    paired_mate_streams = NULL, accession = NULL, sampling_seed = 42L,
    max_reads_per_round = NULL, low_total_reads = 10000L) {
    read_source <- match.arg(read_source)
    detected <- system.time(report <- selexprep_detect(
    ...
    extracted <- system.time(extraction <- selexprep_extract(
        sequences_by_round = sequences_by_round,
        library_report = report,
        paired_mate_streams = paired_mate_streams
    ))
    counted <- system.time(count_tables <- if (identical(report$extraction_mode, "PAIRED_END_SPLIT_PRIMERS")) {
    ...
    quality_controlled <- system.time(qc <- if (identical(report$extraction_mode, "PAIRED_END_SPLIT_PRIMERS")) {
        NULL
    } else {
    ...
    manifest <- build_selexprep_manifest(
        ...
        parameters = c(
            ...
            max_reads_per_round = if (is.null(max_reads_per_round)) "all" else as.character(max_reads_per_round),
