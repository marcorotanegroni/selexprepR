.as_selexprep_count_table <- function(counts) {
    ...
    if (!identical(colnames(counts), expected)) {
        stop("Counts must have the stable columns: sequence, reads, rank, rpm.",
            call. = FALSE)
    }
    ...
    if (anyNA(sequence) || anyNA(reads) || anyNA(rank) || anyNA(rpm) || any(!is.finite(reads)) ||
        any(!is.finite(rank)) || any(!is.finite(rpm)) || any(reads < 0) || any(rank <
        1) || any(rank != floor(rank))) {
    ...
    expected_rpm <- if (total_reads)
        reads/total_reads * 1e+06 else numeric()
    if (length(rpm) && any(abs(rpm - expected_rpm) > 1e-07 * pmax(1, expected_rpm))) {
        stop("Count RPM values do not agree with the read totals.", call. = FALSE)
    }
    S4Vectors::DataFrame(sequence = Biostrings::BStringSet(sequence), reads = reads,
        rank = as.integer(rank), rpm = rpm)
}
.count_path_format <- function(path) {
    ...
    if (grepl("\\.parquet$", name))
        return("parquet")
    stop("Unsupported count-file extension; use .tsv, .csv, .rds, or .parquet.",
        call. = FALSE)
}
.require_arrow <- function() {
    if (!requireNamespace("arrow", quietly = TRUE)) {
        stop("Parquet support requires the optional `arrow` package.", call. = FALSE)
    }
}
...
read_selexprep_counts <- function(path) {
    ...
    counts <- switch(format, tsv = utils::read.delim(path, check.names = FALSE,
        stringsAsFactors = FALSE), csv = utils::read.csv(path, check.names = FALSE,
        stringsAsFactors = FALSE), rds = readRDS(path), parquet = {
        .require_arrow()
...
write_selexprep_counts <- function(counts, path) {
    ...
    format <- .count_path_format(path)
    payload <- data.frame(sequence = as.character(counts$sequence), reads = as.numeric(counts$reads),
        rank = as.integer(counts$rank), rpm = as.numeric(counts$rpm), check.names = FALSE)
    switch(format, tsv = utils::write.table(payload, path, sep = "\t", quote = FALSE,
        row.names = FALSE), csv = utils::write.csv(payload, path, quote = FALSE,
        row.names = FALSE), rds = saveRDS(counts, path), parquet = {
        .require_arrow()
