...
.top_kmer_fraction <- function(sequences, reads, k = 6L, top_sequences = 10000L) {
    ...
    for (index in selected) {
        ...
        kmers <- substring(sequence, seq_len(width - k + 1L), seq_len(width - k + 1L) + k - 1L)
    ...
    if (!total) 0 else max(kmer_counts) / total
}
...
# Sample a count table without replacement via sequential hypergeometric draws.
    ...
    .with_qc_seed(seed, {
        ...
        if (length(counts) > 1L) {
            for (index in seq_len(length(counts) - 1L)) {
                ...
                sampled[[index]] <- stats::rhyper(
                    1L,
                    m = successes,
                    n = remaining_population - successes,
                    k = remaining_draws
                )
...
.canonical_kmer <- function(kmer) {
    ...
    if (kmer < reverse) kmer else reverse
}
...
.round_kmer_set <- function(sequences, reads, k = 6L, top_sequences = 10000L) {
    ...
    kmers <- unlist(lapply(sequences[selected], function(sequence) {
        ...
        width <- nchar(sequence)
        if (width < k) return(character())
        vapply(
            substring(sequence, seq_len(width - k + 1L), seq_len(width - k + 1L) + k - 1L),
            .canonical_kmer,
            character(1)
        )
    }), use.names = FALSE)
...
.modal_sequence_length <- function(sequences, reads) {
    ...
    weighted <- vapply(unique_lengths, function(length) sum(reads[lengths == length]), numeric(1))
...
.round_kmer_similarity <- function(counts_by_round, k, top_sequences) {
    ...
    if (length(names_by_round) < 2L) {
        return(S4Vectors::DataFrame(
            from_round = character(), to_round = character(), jaccard_distance = numeric()
        ))
    }
    ...
    S4Vectors::DataFrame(
        from_round = names_by_round[pairs[1L, ]],
        to_round = names_by_round[pairs[2L, ]],
...
.round_kmer_monotonicity_flag <- function(round_similarity) {
    ...
    lookup <- stats::setNames(
        round_similarity$jaccard_distance,
        paste(round_similarity$from_round, round_similarity$to_round, sep = "\r")
    )
    ...
    for (start in seq_len(length(round_names) - 2L)) {
        ...
        for (end in seq.int(start + 2L, length(round_names))) {
            ...
            if (!is.null(neighbouring) && !is.null(distant) && neighbouring > distant * 1.1) {
                violations <- append(violations, list(list(
                    from_round = round_names[[start]],
                    neighbour_round = round_names[[start + 1L]],
                    distant_round = round_names[[end]],
                    consecutive_distance = neighbouring,
                    distant_distance = distant
                )))
            }
        }
    }
    ...
    .qc_flag("round_kmer_nonmonotonic", "warn", list(
        tolerance = 1.1,
...
.adapter_contamination_flag <- function(library_report, experiment) {
    ...
    input_reads <- if (inherits(extraction, "selexprep_extraction")) extraction$input_reads else NULL
    ...
    if (!is.finite(denominator) || denominator <= 0) {
        return(.qc_flag("adapter_contamination_high", "info", list(
    ...
    .qc_flag("adapter_contamination_high", "warn", list(
        max_fraction = 0.05,
        denominator = denominator,
        adapters_above_threshold = lapply(above, function(index) list(
            adapter = names(hits)[[index]],
            hits = as.numeric(hits[[index]]),
            fraction_of_reads = as.numeric(fractions[[index]])
        ))
    ))
}
...
.strand_mix_flag <- function(experiment) {
    ...
    flagged <- lapply(names(distributions), function(round) {
        ...
        if (!total || distribution[["reverse"]] / total <= 0.20) {
            return(NULL)
        }
        list(
            round = round,
            reverse_fraction = as.numeric(distribution[["reverse"]] / total),
            forward = as.integer(distribution[["forward"]]),
            reverse = as.integer(distribution[["reverse"]]),
    ...
    .qc_flag("strand_mix", "warn", list(
        max_reverse_fraction = 0.20,
        rounds_above_threshold = flagged
    ))
}
...
.flags_dataframe <- function(flags) {
    if (!length(flags)) {
        return(S4Vectors::DataFrame(name = character(), severity = character(), evidence = I(list())))
    }
    S4Vectors::DataFrame(
        name = vapply(flags, `[[`, character(1), "name"),
        severity = vapply(flags, `[[`, character(1), "severity"),
        evidence = I(lapply(flags, `[[`, "evidence"))
    )
}
...
selexprep_qc <- function(experiment, library_report = NULL, low_total_reads = 10000L,
    rarefaction_depth = 10000L, rarefaction_seed = 42L, kmer_size = 6L,
    ...
    if (!"counts" %in% SummarizedExperiment::assayNames(experiment) ||
        !"sequence" %in% colnames(SummarizedExperiment::rowData(experiment))) {
        stop("`experiment` must contain a counts assay and rowData$sequence.", call. = FALSE)
    }
    ...
    if (!is.numeric(low_total_reads) || length(low_total_reads) != 1L ||
        is.na(low_total_reads) || low_total_reads < 0L) {
    ...
    per_round <- S4Vectors::DataFrame(
        round = names(counts_by_round),
        n_reads = unname(totals),
        n_unique = unname(vapply(counts_by_round, length, integer(1))),
        shannon_entropy_bits = unname(vapply(counts_by_round, .shannon_entropy, numeric(1))),
        rarefied_unique = unname(rarefied_unique),
        singleton_fraction = unname(vapply(counts_by_round, .singleton_fraction, numeric(1))),
        top_1_coverage = unname(vapply(counts_by_round, .top_n_coverage, numeric(1), n = 1L)),
        top_100_coverage = unname(vapply(counts_by_round, .top_n_coverage, numeric(1))),
        modal_sequence_length = unname(vapply(counts_by_round, function(counts) {
            .modal_sequence_length(names(counts), counts)
        }, numeric(1))),
        mean_gc = unname(vapply(gc, `[[`, numeric(1), "mean")),
        gc_sd = unname(vapply(gc, `[[`, numeric(1), "sd")),
        nonstandard_alphabet_fraction = unname(vapply(counts_by_round, function(counts) {
            .weighted_bad_alphabet_fraction(names(counts), counts)
        }, numeric(1))),
        truseq_reads_fraction = unname(vapply(counts_by_round, function(counts) {
            if (!sum(counts)) return(0)
            sum(counts[grepl("AGATCGGAAGAGC", toupper(names(counts)), fixed = TRUE)]) / sum(counts)
        }, numeric(1))),
        top_kmer_fraction = unname(vapply(counts_by_round, function(counts) {
            .top_kmer_fraction(names(counts), counts, k = kmer_size, top_sequences = kmer_top_sequences)
        }, numeric(1)))
    )
    round_similarity <- .round_kmer_similarity(
        counts_by_round,
        k = as.integer(kmer_size),
    ...
    if (length(low_depth)) {
        flags <- append(flags, list(.qc_flag("low_total_reads", "warn", list(
            threshold = low_total_reads,
            rounds = as.list(per_round$round[low_depth]),
            reads = as.list(per_round$n_reads[low_depth])
        ))))
    }
    modal_lengths <- unique(per_round$modal_sequence_length[!is.na(per_round$modal_sequence_length)])
    if (length(modal_lengths) > 2L) {
        flags <- append(flags, list(.qc_flag("n_length_variation_across_rounds", "warn", list(
            modal_lengths = as.list(per_round$modal_sequence_length),
    ...
    if (length(counts_by_round) >= 2L && !is.na(effective_rarefaction_depth)) {
        ...
        if (length(increases)) {
            flags <- append(flags, list(.qc_flag("unexpected_rarefied_diversity_increase", "warn", list(
                configured_depth = as.integer(rarefaction_depth),
                effective_depth = effective_rarefaction_depth,
                rarefied_unique_per_round = as.list(stats::setNames(
                    per_round$rarefied_unique,
                    per_round$round
                )),
                increases = lapply(increases, function(index) list(
                    from_round = per_round$round[[index]],
                    to_round = per_round$round[[index + 1L]],
                    rarefied_unique_prev = per_round$rarefied_unique[[index]],
    ...
    if (length(nonstandard)) {
        flags <- append(flags, list(.qc_flag("nonstandard_alphabet", "warn", list(
            threshold = 0.001,
            rounds = as.list(per_round$round[nonstandard]),
            fractions = as.list(per_round$nonstandard_alphabet_fraction[nonstandard])
        ))))
    }
    truseq <- which(per_round$truseq_reads_fraction > 0.01)
    if (length(truseq)) {
        flags <- append(flags, list(.qc_flag("truseq_residual", "warn", list(
            threshold = 0.01,
            rounds = as.list(per_round$round[truseq]),
            fractions = as.list(per_round$truseq_reads_fraction[truseq])
        ))))
    }
    ...
    if (!is.null(library_report)) {
        low_sides <- character()
        if (library_report$match_rate_5p < 0.4) low_sides <- c(low_sides, "5p")
        if (library_report$match_rate_3p < 0.4) low_sides <- c(low_sides, "3p")
        if (length(low_sides)) {
            flags <- append(flags, list(.qc_flag("low_primer_match", "warn", list(
                threshold = 0.4, sides = as.list(low_sides)
            ))))
        }
        if (identical(library_report$required_action, "READ_MERGING_RECOMMENDED")) {
            flags <- append(flags, list(.qc_flag("requires_read_merging_for_full_insert", "info", list(
    ...
    structure(list(
        ...
        flags = .flags_dataframe(flags),
        settings = list(
            rarefaction_depth = as.integer(rarefaction_depth),
            effective_rarefaction_depth = effective_rarefaction_depth,
            rarefaction_seed = as.integer(rarefaction_seed),
            kmer_size = as.integer(kmer_size),
            kmer_top_sequences = as.integer(kmer_top_sequences)
        )
    ),
